...
- Employee list/create/update/deactivate.
- Attendance check-in/check-out, today/history/team summary, and location metadata.
- Leave types, balances, application, cancellation, and approval/rejection.
- WFH requests and grievance workflows.
- Desktop consent, heartbeat, activity snapshots, app usage, live state, and team summaries.
- Payroll generation, approval, and employee payslip listing.
...
- Admin payroll generation/approval, projects/tasks, and desktop activity views backed by the same company data.
- Landing registration now includes password setup, verification-code activation, and admin-login handoff.
- Mobile work assignments, refresh, backend logout, geofence enforcement, seeded leave, and seeded payslip data.
- Mobile leave, WFH, grievance/support self-service for every role, plus direct-team attendance and leave/WFH approvals for Manager, HR, and Company Admin.

## Validation Completed

- Backend syntax check: pass.
- Backend integration tests: 12/12 pass, including registration-to-admin-to-employee shared data, role permissions, manager direct-team approval flows, Super Admin company editing/archival and cross-company employee management, approval separation, billing ledgers, partial-payment verification, automatic-only pausing, reminder scheduling, paid-seat enforcement, subscriptions, leads, audit history, and tenant access enforcement.
