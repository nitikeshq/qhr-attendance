...
function defaultPayrollSettings(company = {}) {
  return {
    ...
    // Calendar days, because a monthly salary already covers weekly offs: a full
    // July genuinely is 31 paid of 31. Switching this to `working_days` does not
    // change a full month's pay, it makes each loss-of-pay day cost more
    // (gross/23 rather than gross/31), so it is a deliberate policy choice rather
    // than a default. The basis is now shown on the payslip and is editable beside
    // the work week, which is what was actually missing.
...
function buildPayslipHtml(data, sourcePayslip) {
  ...
  // A payslip is a wage record, not a company profile. Only identifiers the
  // employee can actually act on are printed: GSTIN has no payroll meaning at all,
  // and the employer PAN is not the number used to reconcile TDS. Everything else
  // stays in payroll settings, where the calculations and filings still use it.
  const deducted = (code) => deductions.some((item) => item.code === code && amount(item.amount) !== 0);
  const contributed = (code) => employerContributions.some((item) => item.code === code && amount(item.amount) !== 0);
  const pfApplies = deducted('provident_fund') || contributed('employer_epf') || contributed('employer_eps');
  const esiApplies = deducted('employee_state_insurance') || contributed('employer_esi');
  const tdsApplies = deducted('tds');
  const companyRegistrations = tdsApplies && company.tan ? `TAN ${company.tan}` : '';
  ...
  const workLocation = payslip.workLocationSnapshot
    ...
    || null;
  // An establishment code only matters to somebody whose pay it affects, so it is
  // printed when that deduction is actually on this payslip.
  const pfCode = workLocation?.pfEstablishmentCode || company.pfEstablishmentCode || '';
  const esiCode = workLocation?.esiEmployerCode || company.esiEmployerCode || '';
  const establishmentIds = [
    pfApplies && pfCode ? `PF: ${pfCode}` : '',
    esiApplies && esiCode ? `ESI: ${esiCode}` : '',
  ].filter(Boolean).join(' · ');
...
<header class="header"><div class="brand"><div class="logo">${escapeHtml(initials)}</div><div class="company"><h1>${escapeHtml(legalName)}</h1>${workLocation && workLocation.address
  ? `<p><span class="tag">Place of work</span> ${escapeHtml(workLocation.address)}</p>`
  : `<p><span class="tag">Regd. office</span> ${valueOrDash(company.registeredAddress || company.address)}</p>`}${companyRegistrations ? `<p>${escapeHtml(companyRegistrations)}</p>` : ''}${establishmentIds ? `<p>${escapeHtml(establishmentIds)}</p>` : ''}</div></div><div class="document"><h2>PAYSLIP</h2><p><strong>${escapeHtml(periodLabel)}</strong></p><p>${escapeHtml(periodStart)} to ${escapeHtml(periodEnd)}</p><p>Payroll no: <strong>${valueOrDash(payslip.payrollNumber)}</strong></p><p>Document ID: <strong>${escapeHtml(documentId)}</strong></p><span class="pill">${escapeHtml(statusText(payslip.paymentStatus || payslip.status))}</span></div></header>
